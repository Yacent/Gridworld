...
public class MazeBug2 extends Bug {
	 ...
	 *
	 * Improving version
	 * tell the direction to move proceed
	 * using dirCount to record the direction it move
	 * choose the max value of the direction to move
	 */
	public boolean canMove() {
		...
		if (nextMove.size() != 0) {
			Location moveDir = null;
			int max = 0;
			for (Location l : nextMove) {
				int index = loc.getDirectionToward(l) / 90;
				if (dirCount[index] > max) {
					moveDir = l;
					max = dirCount[index];
				}
			}
			...
			next = moveDir;
			int index = loc.getDirectionToward(moveDir) / 90;
			dirCount[index]++;
			isBack = false;
			return true;
		} else {
