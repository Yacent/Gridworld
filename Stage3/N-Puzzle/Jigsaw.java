...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		int maxNumNode = 10000;
		isCompleted = false;
		
		// 记录与v邻接的节点
		Vector<JigsawNode> neighborNode = new Vector<JigsawNode>();
		
		// 排序beginJNode 插入 openList
		this.sortedInsertOpenList(beginJNode);
		
		while (this.openList.size() > 0 && searchedNodesNum <= maxNumNode) {
			
			currentJNode = openList.elementAt(0);
			
			// if currentJNode equals to endJNode, end searching, or go on
			...
			// 按照当前顺序插入openList中
			neighborNode = findFollowJNodes(currentJNode);
			while (neighborNode.size() > 0) {
				openList.addElement(neighborNode.elementAt(0));
				neighborNode.removeElementAt(0);
			}
	...
	public boolean ASearch() throws IOException{
		...
		int threshold = beginJNode.getEstimatedValue();
		*/
